        ...
        (err, book_r) => {
            if (err) {
                ...
                if (book_r) {
                    User.findOne({ _id: book_r.userID }, async(err, user_r) => {
                        if (err) return next(err);
                        mailer = await Mailer({ email: user_r.email, subject: `${book_r.name} book request cancelled by ${user_r.name}`, profile: `<table><tr><td><img src="${user_r.photo}" style="height:50px;width:50px;border-radius:100%"></td></tr><tr><td><h1>${user_r.name}</h1></td></tr><tr><td><h2>Book name:${book_r.name}</h2></td></tr><tr><td><h3 style="margin:1">${message}</h3></td></tr></table>` });
                        if (mailer.err) {
                            err = mailer.err
                            err.code = 111
                            err.message = "Connection refused or inability to open an SMTP stream"
